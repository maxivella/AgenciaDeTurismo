...
public class Svmodificarpaquete extends HttpServlet {
 ...
 try{       
    ...
    List<Servicio> serviciosDelPaquete= pasarAlista(request.getParameter("idservicios"));
 
     Float costo= !request.getParameter("costodepaquete").isEmpty()?
                   Float.parseFloat(request.getParameter("costodepaquete"))
                  :calcularCosto(serviciosDelPaquete);      
 if(serviciosDelPaquete.size()>=2)       
 {clp.editarPaquete(idpaquete, costo, nombreUnico, serviciosDelPaquete);
  PrintWriter out = response.getWriter();
  out.println("ENVIADO AL SERVIDOR"); 
 }
 else{response.getWriter().println("PAQUETES DEBEN CONTENER AL MENOS DOS SERVICIOS"); }       
